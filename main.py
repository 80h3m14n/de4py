...
def cupdate() -> None:
    if update.check_update():
        ...
        logging.warning(
            "There's a new version. Are you sure you want to use this version?")
        tui.fade_type('Answer [y/n]\n')
        if not input(">>> ").lower() == 'y':
            logging.warning(
                "Download it from here : https://github.com/Fadi002/de4py")
...
def file_explorer() -> str:
    ...
    file_path = filedialog.askopenfilename(filetypes=[(
        "Python Files", "*.py"), ("Python compiled Files", "*.pyc"), ("exe Files", "*.exe"), ("All Files", "*.*")])
    return file_path
...
def get_info() -> str:
    ...
    if not arch.startswith('64'):
        logging.warning(
            "your pc arch is not x64 bit please note this tool was tested on windows x64 bit")
...
def dumpsocketcontent(var) -> str:
    if var:
        dealwithfilesocket()
        response = write_to_pipe(
            "DumpConnections||" + os.getcwd() + "\\SocketDump.txt")
...
def update_hooks_output():
    ...
    try:
        ...
        logging.error(
            f"Error occurred while reading from HANDLE_analyzer: {str(e)}")
    ...
    if config.__LOAD_PLUGINS__:
        ...
        for plugin in loaded_plugins:
            if plugin["type"] == "deobfuscator":
                ...
                obfuscators.append(
                    (plugin.plugin_name, plugin.regex, plugin.deobfuscator_function))
                plugins.build_html(
                    {"name": plugin.plugin_name, "creator": plugin.creator, "link": plugin.link})
                logging.info(
                    f"Loaded deobfuscator plugin {plugin.plugin_name}")
...
def main() -> None:
    if config.__STEALTH_TITLE__:
        ...
        if platform.system() == "Windows":
            import random
            import string
            ctypes.windll.kernel32.SetConsoleTitleW([''.join(random.choices(
                string.ascii_letters + string.digits, k=random.randint(10, 40))) for _ in range(random.randint(10, 40))][0])
    else:
    ...
    try:
        ...
        if "Chrome" in str(e) or "Chromium" in str(e):
            logging.warning(
                "Chrome/Chromium not found. Starting in CLI mode instead...")
            logging.info(
                "Use 'python main.py --cli' to start directly in CLI mode")
            from TUI import cli
            cli.start()
        else:
            raise
