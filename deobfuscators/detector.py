...
obfuscators = [
    ...
    ("PlusOBF", re.compile(
        r"exec\(\"\"\.join\(\[chr\(len\(i\)\) for i in d\]\)\)"), PlusOBF),
    ("wodx", re.compile(r'(?:__NO_NO){23}'), wodx),
    ("BlankOBF", re.compile(
        r"import\s*base64,\s*lzma;\s*exec\(compile\(lzma\.decompress\(base64\.b64decode\(b'([A-Za-z0-9+/=]+)'\)\)\s*,\s*\"<string>\"\s*,\s*\"exec\"\)\)"), BlankOBF),
    ("Hyperion", re.compile(
        r'__obfuscator__\s*=\s*[\'\"]\s*Hyperion\s*[\'\"]'), Hyperion),
    ('jawbreaker', re.compile(r'([a-zA-Z_]\w{3})\s*=\s*([^;]+);'), jawbreaker),
    ('freecodingtools', re.compile(
        r"^_\s*=\s*lambda\s+__\s*:\s*__import__\('zlib'\)\.decompress\(__import__\('base64'\)\.b64decode\(__\[::-1]\)\);exec\(\(\_\)"), freecodingtools),
